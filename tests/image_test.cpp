...
TEST_CASE_METHOD(ImageTestEnv, "LoadTextureFromMemory (PNG buffer)", "[image][memory]")
{
    ...
    leo_Texture2D tex = leo_LoadTextureFromMemory(".png", reinterpret_cast<const unsigned char *>(buf.data()),
...
TEST_CASE_METHOD(ImageTestEnv, "LoadTextureFromPixels RGBA8 (direct upload)", "[image][pixels]")
{
    const int W = 2, H = 2;
    // 2x2 checker RGBA
    unsigned char px[W * H * 4] = {255, 0, 0, 255, 0, 255, 0, 255, 0, 0, 255, 255, 255, 255, 0, 255};
...
{
    const int W = 3, H = 1;
    unsigned char px[W * 3] = {10, 20, 30, 40, 50, 60, 70, 80, 90};
...
{
    const int W = 2, H = 2;
    unsigned char px[W * H * 2] = {0, 255, 255, 128, 127, 64, 200, 0};
