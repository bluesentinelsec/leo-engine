...
static SDL_GamepadButton s_map_button(int b) {
    switch ((leo_GamepadButton)b) {
        ...
        case LEO_GAMEPAD_BUTTON_BACK:         return SDL_GAMEPAD_BUTTON_BACK;
        case LEO_GAMEPAD_BUTTON_GUIDE:        return SDL_GAMEPAD_BUTTON_GUIDE;
        case LEO_GAMEPAD_BUTTON_START:        return SDL_GAMEPAD_BUTTON_START;
...
#else
        if (!gpads[i].pad && !gpads[i].test_pad) return i;
#endif
    }
    return -1;
}
...
static void s_update_buttons_for_slot(int i) {
    ...
    for (int b = 0; b < LEO_GAMEPAD_BUTTON_COUNT; ++b) {
        ...
        if (sb < 0) continue;
        if (SDL_GetGamepadButton(s->pad, sb)) mask |= s_btn_bit(b);
    }
#else
    mask = s->test_buttons;
#endif
...
void leo_ShutdownGamepads(void) {
    for (int i = 0; i < LEO_MAX_GAMEPADS; ++i) s_close_slot(i);
}
...
bool leo_IsGamepadAvailable(int gamepad) {
    if (gamepad < 0 || gamepad >= LEO_MAX_GAMEPADS) return false;
#ifndef LEO_TEST_BACKEND
...
float leo_GetGamepadAxisMovement(int gamepad, int axis) {
    ...
    if (axis < 0 || axis >= (int)LEO_GAMEPAD_AXIS_COUNT) return 0.0f;
...
static inline int s_check_stick_args(int gamepad, int stick, int dir) {
    if (gamepad < 0 || gamepad >= LEO_MAX_GAMEPADS) return 0;
#ifndef LEO_TEST_BACKEND
    if (!gpads[gamepad].pad) return 0;
#else
    if (!gpads[gamepad].test_pad) return 0;
#endif
...
void leo__test_detach_pad(int index) {
    if (index < 0 || index >= LEO_MAX_GAMEPADS) return;
    s_close_slot(index);
}
...
void leo__test_set_button(int index, int button, bool down) {
    ...
    LeoGamepadSlot* s = &gpads[index];
    if (!s->test_pad) return;
    if (button < 0 || button >= LEO_GAMEPAD_BUTTON_COUNT) return;

    uint32_t bit = s_btn_bit(button);
    if (down) s->test_buttons |= bit;
    else      s->test_buttons &= ~bit;
}
...
void leo__test_set_axis(int index, int axis, float value) {
    ...
    LeoGamepadSlot* s = &gpads[index];
    if (!s->test_pad) return;
    if (axis < 0 || axis >= (int)LEO_GAMEPAD_AXIS_COUNT) return;
