...
Leo Engine treats game logic and content as external data. All filesystem access is mediated through **PhysicsFS (PhysFS)**, which provides a virtual filesystem abstraction over directories and archive formats.

At runtime, the engine mounts one or more content roots using PhysFS. These may include:

- A development directory tree on disk.
- One or more packaged archives supported by PhysFS.

From the engine’s perspective, all assets are accessed through a unified virtual filesystem namespace.

The engine provides command-line arguments that allow callers to override:

- The path(s) to mount as content roots.
- The script used as the game’s entry point.

This enables flexible workflows for local development, automated testing, CI pipelines, and distribution without changing engine code.
...
Leo Engine also depends on **PhysFS** for filesystem abstraction and asset access.
...
The engine exposes SDL3’s log routing mechanism so callers may redirect logs (for example, to files or custom sinks).
...
- Avoid C++ features that implicitly allocate outside SDL’s allocator where reasonably possible.
...
- **Deterministic RNG**: the engine provides a single deterministic RNG stream seeded at startup.
- **Stable update boundaries**: all gameplay mutation occurs inside `SimTick()`; the engine avoids hidden iteration over game objects.
...
- Game data consists of media files, Lua scripts, shaders, and Tiled maps accessed through PhysFS.
...
2. **Lua API Surface**
   ...
   API should follow the open/closed principle.

3. **Save Games vs Demos**
...
4. **Threading Future**
   ...
   Planned approach: wrap SDL async I/O; assets are typically loaded in scene constructors. Async completions must be applied deterministically at simulation boundaries.
