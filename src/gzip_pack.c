...
/* Use mmx sdefl/sinfl RAW helpers (no wrapper).
 * NOTE: Do NOT define SDEFL_IMPLEMENTATION/SINFL_IMPLEMENTATION here.
 * They should already be compiled elsewhere (e.g., pack_compress.c).
 */
#include <mmx/sdefl.h> /* sdeflate, SDEFL_LVL_MIN/MAX/DEF */
#include <mmx/sinfl.h> /* sinflate */

/* ---------------- CRC32 ----------------
 * Standard IEEE 802.3 polynomial 0xEDB88320, initial 0xFFFFFFFF, final ~.
 * Small, fast table version.
 */
static const uint32_t LEO_CRC32_TABLE[256] = {
...
static uint32_t leo_crc32(const void *data, size_t len)
{
    const uint8_t *p = (const uint8_t *)data;
    uint32_t crc = 0xFFFFFFFFu;
    for (size_t i = 0; i < len; ++i)
        crc = LEO_CRC32_TABLE[(crc ^ p[i]) & 0xFFu] ^ (crc >> 8);
    return ~crc;
}
...
leo_pack_result leo_compress_gzip(const void *in, size_t in_sz, void *out, size_t *out_sz, int level)
{
    if (!in || !out || !out_sz)
        return LEO_PACK_E_ARG;
    if (in_sz > (size_t)INT_MAX)
        return LEO_PACK_E_ARG; /* sdefl uses int */

    const size_t need = leo_gzip_bound(in_sz);
    if (*out_sz < need)
    ...
    dst[pos + 0] = 0x1F; /* ID1 */
    dst[pos + 1] = 0x8B; /* ID2 */
    dst[pos + 2] = 0x08; /* CM: deflate */
    dst[pos + 3] = 0x00; /* FLG: no extra/name/comment/hcrc */
    dst[pos + 4] = 0x00; /* MTIME (0) */
    dst[pos + 5] = 0x00;
    dst[pos + 6] = 0x00;
    dst[pos + 7] = 0x00;
    dst[pos + 8] = 0x00; /* XFL */
    dst[pos + 9] = 0xFF; /* OS: unknown */
    pos += 10;
    ...
    if (*out_sz - pos < 8)
        return LEO_PACK_E_NOSPACE;
    const uint32_t crc = leo_crc32(in, in_sz);
    const uint32_t isz = (uint32_t)(in_sz & 0xFFFFFFFFu);
    wr_u32le(dst + pos + 0, crc);
    wr_u32le(dst + pos + 4, isz);
    pos += 8;
...
leo_pack_result leo_decompress_gzip(const void *in, size_t in_sz, void *out, size_t *out_sz)
{
    if (!in || !out || !out_sz)
        return LEO_PACK_E_ARG;
    if (in_sz < 18)
        return LEO_PACK_E_DECOMPRESS; /* minimal header+trailer */
    ...
    p = 10; /* skip fixed header fields (ID1, ID2, CM, FLG, MTIME, XFL, OS) */
    ...
    /* FHCRC (ignore value; just skip 2 bytes if present) */
    if (flg & 0x02)
    {
        if (p + 2 > in_sz)
            return LEO_PACK_E_DECOMPRESS;
    ...
    if (in_sz < p + 8)
        return LEO_PACK_E_DECOMPRESS; /* need at least trailer */
    const size_t trailer_pos = in_sz - 8;
    if (trailer_pos < p)
        return LEO_PACK_E_DECOMPRESS;

    const size_t comp_len = trailer_pos - p;

    /* --- Inflate raw deflate --- */
    int produced = sinflate(out, (int)*out_sz, src + p, (int)comp_len);
    if (produced < 0)
        return LEO_PACK_E_DECOMPRESS;
    ...
    *out_sz = (size_t)produced;
    ...
    /* --- Validate trailer --- */
    const uint8_t *tr = src + trailer_pos;
    uint32_t crc_exp = (uint32_t)(tr[0] | (tr[1] << 8) | (tr[2] << 16) | (tr[3] << 24));
    uint32_t isz_exp = (uint32_t)(tr[4] | (tr[5] << 8) | (tr[6] << 16) | (tr[7] << 24));

    const uint32_t crc_act = leo_crc32(out, *out_sz);
    const uint32_t isz_act = (uint32_t)(*out_sz & 0xFFFFFFFFu);

    if (crc_exp != crc_act || isz_exp != isz_act)
        return LEO_PACK_E_DECOMPRESS;
