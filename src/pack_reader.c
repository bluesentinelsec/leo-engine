...
{
	if (file_seek64(f, 0) != 0) return LEO_PACK_E_IO;

	leo_pack_header_v1 hdr;
	if (fread(&hdr, 1, sizeof(hdr), f) != sizeof(hdr)) return LEO_PACK_E_IO;

	if (memcmp(hdr.magic, LEO_PACK_MAGIC, 8) != 0) return LEO_PACK_E_FORMAT;
	if (hdr.version != LEO_PACK_V1) return LEO_PACK_E_FORMAT;
	...
	if (calc != expect) return LEO_PACK_E_FORMAT;
...
static leo_pack_result load_toc(FILE* f, const leo_pack_header_v1* hdr,
	pack_entry_rec** out_entries, int* out_count)
{
	if (hdr->toc_size == 0)
	{
		*out_entries = NULL;
	...
	if (file_seek64(f, hdr->toc_offset) != 0) return LEO_PACK_E_IO;
...
{
	if (!out || !path) return LEO_PACK_E_ARG;

	FILE* f = fopen(path, "rb");
	if (!f) return LEO_PACK_E_IO;
...
{
	if (!p || !out) return LEO_PACK_E_ARG;
...
{
	if (!p || !name || !out_index) return LEO_PACK_E_ARG;
	for (int i = 0; i < p->count; ++i)
	{
...
{
	if (!p || !dst) return LEO_PACK_E_ARG;
	if (index < 0 || index >= p->count) return LEO_PACK_E_NOTFOUND;
