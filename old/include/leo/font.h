...
extern "C"
{
    ...
    LEO_API leo_Font leo_LoadFontFromMemory(const char *fileType, const unsigned char *data, int dataSize,
                                            int pixelSize);
    ...
    LEO_API void leo_DrawText(const char *text, int posX, int posY, int fontSize, leo_Color color);

    // Draw text using an explicit font with subpixel position and extra spacing
    // between glyphs (in pixels at the target size).
    LEO_API void leo_DrawTextEx(leo_Font font, const char *text, leo_Vector2 position, float fontSize, float spacing,
                                leo_Color tint);

    // Draw text with origin and rotation (degrees). Rotation is applied around
    // 'origin' relative to 'position' (Raylib-style).
    LEO_API void leo_DrawTextPro(leo_Font font, const char *text, leo_Vector2 position, leo_Vector2 origin,
                                 float rotation, float fontSize, float spacing, leo_Color tint);
    ...
    LEO_API leo_Vector2 leo_MeasureTextEx(leo_Font font, const char *text, float fontSize, float spacing);
