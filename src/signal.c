...
static void leo__lock(leo_SignalEmitter *se)
{
    if (se && se->_lock)
        SDL_LockMutex((SDL_Mutex *)se->_lock);
}
static void leo__unlock(leo_SignalEmitter *se)
{
    if (se && se->_lock)
        SDL_UnlockMutex((SDL_Mutex *)se->_lock);
}
