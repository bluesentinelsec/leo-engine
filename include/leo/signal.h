 ...
 *  - Thread-safe: each emitter has an internal lock (opaque).
 */
...
/* Linked-list node for a connected callback */
struct leo_Callback
{
...
struct leo_SignalEmitter
{
	...
	void* _lock; /* opaque pointer to SDL_Mutex (hidden in signal.c) */
};
...
LEO_API void leo_signal_connect(leo_SignalEmitter* se,
	const char* signal_name,
	leo_SignalCallback callback,
	void* user_data);

LEO_API void leo_signal_disconnect(leo_SignalEmitter* se,
	const char* signal_name,
	leo_SignalCallback callback,
	void* user_data);
