...
static fs::path out_path(const char* name){ return fs::temp_directory_path() / name; }
...
TEST_CASE("reader: extract by name, count/stat bounds, NOTFOUND")
{
	...
	leo_pack_build_opts opt{}; REQUIRE(leo_pack_writer_begin(&w, outP.string().c_str(), &opt) == LEO_PACK_OK);
	...
	leo_pack* p = nullptr; REQUIRE(leo_pack_open_file(&p, outP.string().c_str(), nullptr) == LEO_PACK_OK);
