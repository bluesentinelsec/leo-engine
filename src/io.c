...
static int _is_bad_logical(const char* name)
{
	...
	{
		if (p[0] == '.' && p[1] == '.') return 1;
		if (*p == '\\') return 1; /* backslashes not allowed */
	}
	return 0;
}
...
{
	if (!out || outcap == 0 || !base || !rel) return 0;
...
{
	if (_is_bad_logical(logicalName)) return false;
	if (out)
	{
		out->size = 0;
		out->from_pack = 0;
	...
	for (int i = 0; i < s_count; ++i)
	{
		...
		{
			char full[4096];
			if (!_join_path(full, sizeof(full), (const char*)m->impl, logicalName)) continue;
...
size_t leo_ReadAsset(const char* logicalName, void* buffer, size_t bufferCap, size_t* out_total)
{
	...
	for (int i = 0; i < s_count; ++i)
	{
		...
		{
			char full[4096];
			if (!_join_path(full, sizeof(full), (const char*)m->impl, logicalName)) goto next_mount;
			FILE* f = fopen(full, "rb");
			if (!f) goto next_mount;
...
bool leo_AssetSeek(leo_AssetStream* s, long long off, leo_SeekWhence whence)
{
	...
	{
		size_t base = 0;
		if (whence == LEO_SEEK_SET) base = 0;
		else if (whence == LEO_SEEK_CUR) base = s->pos;
		else if (whence == LEO_SEEK_END) base = s->size;
		else return false;

		long long t = (long long)base + off;
		if (t < 0) return false;
		size_t np = (size_t)t;
		if (np > s->size) return false;
		s->pos = np;
		return true;
	}
	else
...
{
	if (!s) return -1;
	if (s->from_pack) return (long long)s->pos;
	long p = ftell(s->f);
...
char* leo_LoadTextAsset(const char* logicalName, size_t* out_size_without_nul)
{
	...
	if (!p)
	{
		if (out_size_without_nul) *out_size_without_nul = 0;
		return NULL;
	}
	...
	memcpy(s, p, n);
	s[n] = '\0';
	if (out_size_without_nul) *out_size_without_nul = n;
	free(p);
	return s;
}
