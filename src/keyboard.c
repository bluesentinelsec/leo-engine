...
{
	if (s_keyboardInitialized) return;
...
void leo_UpdateKeyboard(void)
{
	...
	if (!s_testMode)
	{
		...
		if (sdl && len > 0)
		{
			...
			for (int i = 0; i < n; ++i) s_currentKeys[i] = (sdl[i] != 0);
		}
	}
#else
    int len = 0;
...
static SDL_Scancode key_to_scancode(int key)
{
	...
	else
	{
		...
		if (sc == SDL_SCANCODE_UNKNOWN) sc = (SDL_Scancode)key;
		return sc;
	}
}
...
{
	leo_InitKeyboard();
	if (!s_keyboardInitialized) return false;

	SDL_Scancode sc = key_to_scancode(key);
	if (sc < 0 || sc >= s_numKeys) return false;
...
{
	leo_InitKeyboard();
	if (!s_keyboardInitialized) return true;

	SDL_Scancode sc = key_to_scancode(key);
	if (sc < 0 || sc >= s_numKeys) return true;
	return !s_currentKeys[sc];
}
...
{
	leo_InitKeyboard();
	if (!s_keyboardInitialized) return 0;
...
int leo_GetCharPressed(void)
{
	...
	if (key >= 32 && key <= 126) return key; // printable ASCII
	return 0;
}
...
// ---------------------- TESTING helpers ---------------------
#ifdef TESTING
void leo_EnableTestMode(void) { s_testMode = true; }
void leo_DisableTestMode(void) { s_testMode = false; }

void leo_SetTestKeyState(SDL_Scancode scancode, bool pressed)
{
	leo_InitKeyboard();
	if (!s_keyboardInitialized) return;
	if (scancode < 0 || scancode >= s_numKeys) return;
	s_currentKeys[scancode] = pressed;
}

// Accessors to avoid DLL data imports on Windows
int leo__test_num_keys(void) { return s_numKeys; }
const bool* leo__test_current(void) { return s_currentKeys; }
const bool* leo__test_prev(void) { return s_prevKeys; }
#endif
