...
{
	if (lvl < SDEFL_LVL_MIN) return SDEFL_LVL_MIN;
	if (lvl > SDEFL_LVL_MAX) return SDEFL_LVL_MAX;
	return lvl;
}
...
/* ----------------------------
   ...
   ---------------------------- */
leo_pack_result
leo_compress_deflate(const void* in, size_t in_sz, void* out, size_t* out_sz,
...
/* ----------------------------
   ...
   ---------------------------- */
leo_pack_result
leo_decompress_deflate(const void* in, size_t in_sz, void* out, size_t* out_sz)
{
	if (!in || !out || !out_sz) return LEO_PACK_E_ARG;
