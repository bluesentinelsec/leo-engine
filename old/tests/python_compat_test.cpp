...
TEST_CASE("Python leo-packer compatibility", "[pack][python]")
{
    ...
    SECTION("Can extract files from Python-created archive")
    {
        ...
        for (int i = 0; i < std::min(count, 3); i++)
        {
            ...
            leo_pack_result extract_result =
                leo_pack_extract_index(pack, i, buffer.data(), buffer.size(), &extracted_size);
    ...
    SECTION("Wrong password fails correctly")
    {
        ...
        if (result == LEO_PACK_OK)
        {
            ...
            uint8_t buffer[1024];
            size_t extracted_size;
            leo_pack_result extract_result = leo_pack_extract_index(pack, 0, buffer, sizeof(buffer), &extracted_size);
    ...
    SECTION("Can find specific files by name")
    {
        ...
        size_t extracted_size;
        leo_pack_result extract_result =
            leo_pack_extract(pack, stat.name, buffer.data(), buffer.size(), &extracted_size);
