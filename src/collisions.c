...
static LEO_INLINE float leo__minf(float a, float b) { return (a < b) ? a : b; }
static LEO_INLINE float leo__maxf(float a, float b) { return (a > b) ? a : b; }
...
static LEO_INLINE float leo__dot2(leo_Vector2 a, leo_Vector2 b) { return a.x*b.x + a.y*b.y; }
static LEO_INLINE float leo__len2_sq(leo_Vector2 a) { return a.x*a.x + a.y*a.y; }

static LEO_INLINE leo_Vector2 leo__sub2(leo_Vector2 a, leo_Vector2 b) { leo_Vector2 r; r.x=a.x-b.x; r.y=a.y-b.y; return r; }
static LEO_INLINE leo_Vector2 leo__add2(leo_Vector2 a, leo_Vector2 b) { leo_Vector2 r; r.x=a.x+b.x; r.y=a.y+b.y; return r; }
static LEO_INLINE leo_Vector2 leo__scale2(leo_Vector2 a, float s) { leo_Vector2 r; r.x=a.x*s; r.y=a.y*s; return r; }

/* Rectangle helpers */
static LEO_INLINE float rec_left (leo_Rectangle r) { return r.x; }
static LEO_INLINE float rec_right(leo_Rectangle r) { return r.x + r.width; }
static LEO_INLINE float rec_top  (leo_Rectangle r) { return r.y; }
static LEO_INLINE float rec_bot  (leo_Rectangle r) { return r.y + r.height; }
...
bool leo_CheckCollisionRecs(leo_Rectangle a, leo_Rectangle b)
{
    ...
    if (rec_right(a) < rec_left(b))  return false;
    if (rec_right(b) < rec_left(a))  return false;
    if (rec_bot(a)   < rec_top(b))   return false;
    if (rec_bot(b)   < rec_top(a))   return false;
    return true;
}
...
bool leo_CheckCollisionCircleLine(leo_Vector2 center, float radius,
...
{
    return (p.x >= rec_left(r) - LEO_EPS) && (p.x <= rec_right(r) + LEO_EPS) &&
           (p.y >= rec_top(r)  - LEO_EPS) && (p.y <= rec_bot(r)   + LEO_EPS);
}
...
bool leo_CheckCollisionLines(leo_Vector2 a1, leo_Vector2 a2,
                             leo_Vector2 b1, leo_Vector2 b2,
                             leo_Vector2* out_pt)
{
    ...
    if (fabsf(den) <= LEO_EPS) {
        ...
        if (fabsf(cross) > 1e-4f) return false; /* parallel, non-overlapping */
    ...
    if (t < -LEO_EPS || t > 1.0f + LEO_EPS ||
        u < -LEO_EPS || u > 1.0f + LEO_EPS) {
        return false;
    }
