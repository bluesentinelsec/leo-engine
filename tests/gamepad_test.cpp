...
struct TestPad
{
    ...
    {
        leo__test_set_button(idx, b, down);
    }
    void setAxis(int a, float v)
    {
        leo__test_set_axis(idx, a, v);
    }
};
...
TEST_CASE("Stick vector and all directions", "[gamepad][stick]")
{
    ...
    struct
    {
        int stick;
        int axis_x, axis_y;
    } sticks[] = {{LEO_GAMEPAD_STICK_LEFT, LEO_GAMEPAD_AXIS_LEFT_X, LEO_GAMEPAD_AXIS_LEFT_Y},
