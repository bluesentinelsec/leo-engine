...
{
	while (end > s && (unsigned char)end[-1] <= 0x20) --end; /* ASCII <= space */
	return end;
}

static const char* lskip_ws(const char* s, const char* end)
{
	while (s < end && (unsigned char)*s <= 0x20) ++s;
	return s;
}
...
leo_csv_result leo_csv_parse(const char* data, size_t len,
	const leo_csv_opts* opt_in,
	leo_csv_cell_cb on_cell, void* user)
{
	if (!data || !on_cell) return LEO_CSV_E_ARG;
	...
	while (p < end)
	{
		...
		if (*p == opt.quote)
		{
			...
			const char* q = p;
			while (q < end && (unsigned char)*q <= 0x20 &&
			       *q != opt.delimiter && *q != '\r' && *q != '\n')
				++q;
...
static int on_cell_u32(void* u, const char* cell, size_t len, size_t row, size_t col)
{
	...
	while (s < e && (unsigned char)*s <= 0x20) ++s;
	while (e > s && (unsigned char)e[-1] <= 0x20) --e;
	if (s == e) return 0;
	...
	while (p < e && *p >= '0' && *p <= '9')
	{
		...
		++p;
		any = 1;
		if (v > 0xFFFFFFFFull) v = 0xFFFFFFFFull; /* clamp */
	}
	...
	if (!any)
	{
		...
		while (it < e)
		{
			...
			if (it < e && *it >= '0' && *it <= '9')
			{
				...
				do
				{
					...
					++it;
					if (vv > 0xFFFFFFFFull) vv = 0xFFFFFFFFull; /* clamp */
		...
		if (!any) return 0; /* still nothing numeric; ignore cell */
	}
...
leo_csv_result leo_csv_parse_uint32_alloc(const char* data, size_t len,
	uint32_t** out, size_t* out_count,
	const leo_csv_opts* opt_in)
{
	if (!out) return LEO_CSV_E_ARG;
	*out = NULL;
...
size_t leo_csv_count_values(const char* data, size_t len, const leo_csv_opts* opt_in)
{
	...
	while (p < end)
	{
		...
		if (in_quote)
		{
			if (c == opt.quote)
			{
				if (p < end && *p == opt.quote) { ++p; /* "" */ }
				else in_quote = 0;
			}
		}
		else
		{
			if (c == opt.quote) in_quote = 1;
