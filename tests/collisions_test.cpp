...
{
    GfxEnvCollisions() { REQUIRE(leo_InitWindow(320, 180, "collisions")); }
    ~GfxEnvCollisions() { leo_CloseWindow(); }
};
...
TEST_CASE_METHOD(GfxEnvCollisions, "Segment vs Segment", "[collisions]")
{
    ...
    ok = leo_CheckCollisionLines(leo_Vector2{0,0}, leo_Vector2{100,0},
                                 leo_Vector2{0,10}, leo_Vector2{100,10}, nullptr);
    REQUIRE_FALSE(ok);

    // Collinear overlapping segments -> considered "collision"
    ok = leo_CheckCollisionLines(leo_Vector2{0,0}, leo_Vector2{100,0},
                                 leo_Vector2{50,0}, leo_Vector2{150,0}, &hit);
    REQUIRE(ok);
}
