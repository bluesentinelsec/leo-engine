...
extern "C"
{
     ...
     */
    LEO_API leo_b64_result leo_base64_encode(const void *src, size_t src_len, char *dst, size_t dst_cap,
                                             size_t *out_len);
     ...
     * - Returns LEO_B64_E_NOSPACE if dst_cap is too small.
     */
    LEO_API leo_b64_result leo_base64_decode(const char *src, size_t src_len, void *dst, size_t dst_cap,
                                             size_t *out_len);

    /* Convenience helpers (malloc-based). Caller must free(*out) on success. */
    LEO_API leo_b64_result leo_base64_encode_alloc(const void *src, size_t src_len, char **out, size_t *out_len);

    LEO_API leo_b64_result leo_base64_decode_alloc(const char *src, size_t src_len, unsigned char **out,
                                                   size_t *out_len);
